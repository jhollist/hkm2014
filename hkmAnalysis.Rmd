...
#run for all predictors
	...
	#view results
		varSelAll  #Selected variables: c('NTL','PTL','TOC','TURB','K',WSA_ECO9)
		summary(varSelAll$selec.history)
		plot(varSelAll, which=2)

#run for GIS predictors
	#select complete cases
...
```{r linearModels, echo=FALSE, cache=FALSE} 
	#select data for linear models
		#select vars for linear model
			Vars<-c('CHLA','NTL','PTL','TOC','TURB','K') 
		#Log transform the data
			LMall<-log(hkm2014Data[,Vars]) #log transform data
    ...
    #4. lnCHLA ~ lnNTL + lnPTL + lnTOC + lnTURB + lnK
				Formula<- formula(paste(names(LMall)[1],'~',paste(names(LMall)[2:6], collapse="+"))) 
					LM<-lm(Formula,data=LMall)
						summary(LM) 						#adjR2== 0.6531
						mean(residuals(LM)^2)		#rmse == 0.7900
