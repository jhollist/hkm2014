...
Random forest is a machine learning algorithm that aggregates numerous decision trees in order to obtain a consensus prediction of the response categories.  Bootstrapped sample data is recursively partitioned according to a given random subset of predictor variables and completely grown without pruning.   With each new tree, both the sample data and predictor variable subset is randomly selected.  
...
Using R’s randomForest package, for more details see @breiman2001random, we pass the reduced models selected with varSelRF and caluclate confusion matrices, overall accuracy and Kappa.
