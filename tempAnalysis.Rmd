...
```{r responseSelection, echo=FALSE, cache=FALSE} 
  ...
  threeClass<-c(min(hkm2014Data$CHLA,na.rm=T),
  							quantile(hkm2014Data$CHLA,na.rm=T)[2],
  							quantile(hkm2014Data$CHLA,na.rm=T)[4],
  							max(hkm2014Data$CHLA,na.rm=T))	
	CHLA_3<-cut(hkm2014Data$CHLA,threeClass,c("low","medium","high"),ordered_result=T)
```
...
### Analysis4: ALL VARIABLES (GIS+WQ): Run varSelRF multiple time to see if the same variables are selected each time. Three CHLA Classes (first quartile, 2nd and 3rd quartile, fourth quartile)
