...
### Analysis1: GIS VARIABLES: Run varSelRF multiple time to see if the same variables are selected each time.
...
#run for all GIS predictors
  ...
  summDF
  require(knitr)
  write(kable(summDF),"gis_variable_summary.md")
  lapply(Out,write,"gis_selected_variables.csv",append=T,ncolumns=1000)
...
#run for all predictors (GIS + WQ)
  ...
  summDF
  require(knitr)
  write(kable(summDF),"all_variable_summary.md")
  lapply(Out,write,"all_selected_variables.csv",append=T,ncolumns=10000)
...
=======
### Analysis3:  Alternatively we could use varSelRFBoot to bootstrap the analysis

#Not working so not currently evaulated.

# ```{r run_varSelRFBoot, echo=FALSE, eval=FALSE, cache=TRUE} 
# #run for all GIS predictors
# 	#intial conditions:
...
# ```
# 
# rf<- varSelRFBoot(Pgis[keep,],TS_CHLA[keep],vars.drop.num = 1,
# 											 bootnumber = 2,
# 											 usingCluster = FALSE,
# 											 srf = rfGIS)
